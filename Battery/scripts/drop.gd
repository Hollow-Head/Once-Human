...
@export_enum("Experience Point", "Plastic", "Glass", "Paper", "Metal") var type_of_drop : int
enum drop_id {EXPERIENCE_POINT, PLASTIC, GLASS, PAPER, METAL} 
...
func _pick_up():
	match type_of_drop:
		drop_id.EXPERIENCE_POINT:
			Player.player.add_experience_points(quantityOfPoints)
		drop_id.PLASTIC:
			Player.add_plastic_items(quantityOfPoints)
		drop_id.GLASS:
			Player.add_glass_items(quantityOfPoints)
		drop_id.PAPER:
			Player.add_paper_items(quantityOfPoints)
		drop_id.METAL:
			Player.add_metal_items(quantityOfPoints)
	queue_free()
